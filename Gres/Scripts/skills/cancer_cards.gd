...
var card_name = ""
var info_en = [
	"[color=#fc5a03]Unlocks a mighty +40% chance for Gold to drop as x2[/color]\n\nYour fate bends to wealth: the Gold x2 drop chance is permanently increased by +40%.",
	
	"[color=#fc5a03]Gold can now ascend to x3 with a 10% chance[/color]\n\nWhen fortune strikes, Gold no longer doubles it triples. The old x2 proc is permanently upgraded to x3 at a 10% rate.",
	
	"[color=#fc5a03]+5% boost to ALL drop rates[/color]\n\nA constant blessing of abundance: every drop in the game gains a permanent +5% chance."
]
...
func on_confirm_pressed() -> void:
	...
	match  card_name:
		"card_1":
			GlobalStats.d_g_drop_percent += 40.0
			GlobalSkills.zodiac_cards['cancer']['card_1'] = true
		"card_2":
			GlobalStats.gold_moltipl = 3.0
			GlobalSkills.zodiac_cards['cancer']['card_2'] = true
		"card_3":
			GlobalStats.drop_percent += 5.0
			GlobalStats.craft_drop += 5.0
			GlobalSkills.zodiac_cards['cancer']['card_3'] = true
