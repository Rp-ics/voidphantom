...
func _ready() -> void:
	...
	if bullet_scene == null:
		push_warning("❌ ERRORE FATALE: impossibile caricare il proiettile da " + bullet_path)
	else:
...
func _fire_missile() -> void:
	if missile_scene == null:
		push_error("⚠️ missile_scene non impostata.")
		return
...
func _spawn_from_point(spawn_pos: Vector2, scene: PackedScene) -> void:
	if scene == null:
		push_warning("❌ ERRORE: scena proiettile nulla.")
		return
	
	var bullet = scene.instantiate()
	if bullet == null:
		push_warning("❌ Errore nell'istanza del proiettile!")
		return
...
func _die() -> void:
	is_dead = true
	shielded = true
	Global.mother_right_canon_damaged = true
	match Global.state:
		"PHASE1":
	...
	match Global.state:
		"PHASE1":
			...
			if hp < 4000 and not shielded:
				...
				Global.mother_right_canon_damaged = false
