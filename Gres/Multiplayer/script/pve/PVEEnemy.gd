...
func _init_bot(p: CharacterBody2D, pid: int, diff: Difficulty, weapon_name: String, skin: Dictionary, pname: String):
	...
	# Assicura che il bot possa essere danneggiato
	player.can_be_damaged = true
	player.mp_is_dead = false
	
	# Imposta la vita massima del bot
	player.mp_max_hp = GameModes.get_max_hp_for_mode(GameModes.current_mode)
	player.mp_synced_hp = player.mp_max_hp
	
	if diff == Difficulty.HARD:
		_reaction_time = randf_range(0.1, 0.3)
		_aim_error = randf_range(0.02, 0.08)
		_aggression = randf_range(0.8, 1.0)
	else:
		_reaction_time = randf_range(0.3, 0.6)
		_aim_error = randf_range(0.1, 0.25)
		_aggression = randf_range(0.5, 0.8)
...
func _apply_skin():
	if player.has_method("_apply_skin"):
	...
	match _state:
		...
		"combat":
			if is_instance_valid(target):
				...
				var move_forward = -to_target if player.global_position.distance_to(target.global_position) < 200 else to_target
				_move_dir = (move_forward * _aggression + perp * _strafe_dir * 0.5).normalized()
...
func _update_skills(delta):
	...
	if player.has_method("_trigger_active_skill") and player._skill_ready:
		var dist = player.global_position.distance_to(target.global_position)
		if dist < 500 or randf() < 0.15:
			player._trigger_active_skill()
			_skill_cooldown = 8.0 + randf_range(0, 5.0)
