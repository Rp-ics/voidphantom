...
func _ready():
	...
	# Stessa logica di mu_wings.gd: i player remoti ricevono
	# la texture corretta via _apply_skin() in mp_player.gd.
...
func _process(delta):
	# BUGFIX: Non processare per player remoti.
	# Senza questo controllo, ogni propulsore remoto esegue il codice
	# del dash usando i tasti del computer locale, causando dash
	# indesiderati sul personaggio dell'avversario.
	if not get_parent() or not get_parent().is_multiplayer_authority():
		return
