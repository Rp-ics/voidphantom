...
# === SAVE SYSTEM === #
var CLOUD_SAVE_NAME = "player_data.save"
...
# ======================================
# SAVE SYSTEM - FUNZIONI PRINCIPALI
# ======================================
...
func save_game() -> bool:
	...
	# Prova a salvare su Steam Cloud
	if isRunning:
		if save_to_cloud(data):
			success = true
			print("✅ Salvataggio Steam Cloud completato")
		else:
			print("⚠ Steam Cloud non disponibile")
...
func load_game() -> bool:
	...
	# Prima prova a caricare da Steam Cloud
	if isRunning:
		data = load_from_cloud()
		if not data.is_empty():
			print("✅ Dati caricati da Steam Cloud")
			success = true
	
	# Se cloud fallisce o vuoto, carica da locale
...
func save_to_cloud(data: Dictionary) -> bool:
	...
	if not Steam.isSteamRunning():
		print("⚠ Steam non è in esecuzione")
		return false
	
	var json_data = JSON.stringify(data)
	# Converti stringa in bytes
	var bytes = json_data.to_utf8_buffer()
	
	if Steam.fileWrite(CLOUD_SAVE_NAME, bytes, bytes.size()):
		# Verifica che sia stato scritto
		var size = Steam.getFileSize(CLOUD_SAVE_NAME)
		if size > 0:
			print("📊 File cloud scritto: " + str(size) + " bytes")
			return true
		else:
			print("❌ File cloud scritto ma dimensione 0")
			return false
	else:
		print("❌ Errore nella scrittura del file cloud")
		return false
...
func load_from_cloud() -> Dictionary:
	...
	if not Steam.fileExists(CLOUD_SAVE_NAME):
		print("⚠ File cloud non trovato")
		return {}
	
	var size = Steam.getFileSize(CLOUD_SAVE_NAME)
	if size <= 0:
		print("⚠ File cloud vuoto o corrotto")
		return {}
	
	# CORREZIONE: Steam.fileRead() restituisce un Dictionary in Godot 4
	var result = Steam.fileRead(CLOUD_SAVE_NAME, size)
	
	# Controlla se il risultato è valido
	if result == null or typeof(result) != TYPE_DICTIONARY:
		print("❌ Risultato fileRead non valido: " + str(result))
		return {}
	
	# Il dictionary dovrebbe avere una chiave 'buffer' con i dati
	if not result.has("buffer"):
		print("❌ Dictionary non contiene buffer")
		print("Chiavi disponibili: " + str(result.keys()))
		return {}
	
	var bytes = result["buffer"]
	
	# Verifica che bytes sia un PackedByteArray
	if typeof(bytes) != TYPE_PACKED_BYTE_ARRAY:
		print("❌ Buffer non è PackedByteArray, tipo: " + str(typeof(bytes)))
		return {}
	...
	var json_data = bytes.get_string_from_utf8()
	
	if json_data == "" or json_data == null:
		print("❌ Dati cloud vuoti dopo conversione")
		print("Dimensione bytes: " + str(bytes.size()))
		# Debug: stampa primi bytes
		var debug_str = ""
		for i in range(min(bytes.size(), 20)):
			debug_str += str(bytes[i]) + " "
		print("Primi bytes: " + debug_str)
		return {}
	
	var json = JSON.new()
	var error = json.parse(json_data)
	if error == OK:
		print("✅ JSON cloud parsato correttamente")
		print("Dimensione dati: " + str(json_data.length()) + " caratteri")
		return json.data
	else:
		print("❌ Errore parsing JSON cloud: " + json.get_error_message())
		print("JSON data (primi 200 caratteri): " + json_data.substr(0, min(200, json_data.length())) + "...")
		return {}
# ======================================
# BACKUP SYSTEM
# ======================================
...
func _ready() -> void:
	# Inizializza Steam
	if OS.get_name() in ["Windows", "OSX", "Linux", "X11"]:
		var steam_init = Steam.steamInit()
		if steam_init:
			isRunning = true
			print("✅ Steam inizializzato correttamente")
		else:
			isRunning = false
			print("⚠ Steam non inizializzato, solo salvataggio locale")
	else:
		isRunning = false
		print("⚠ Piattaforma non Steam, solo salvataggio locale")
...
func get_save_status() -> String:
	var status = "Steam Cloud: "
	status += "✅ ATTIVO" if isRunning else "❌ NON ATTIVO"
	status += "\nUltimo salvataggio: "
	if last_save_time > 0:
...
func delete_save():
	...
	# Elimina da Steam Cloud (se disponibile)
	if isRunning and Steam.fileExists(CLOUD_SAVE_NAME):
		if Steam.fileDelete(CLOUD_SAVE_NAME):
			print("🗑️ Salvataggio cloud eliminato")
		else:
			print("❌ Impossibile eliminare salvataggio cloud")
