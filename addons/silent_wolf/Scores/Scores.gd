...
func _on_GetScores_request_completed(result, response_code, headers, body) -> void:
	...
	if status_check:
		...
		var json_body = JSON.parse_string(raw_body)
		
		# PROTEZIONE: Se il body non è un dizionario valido, interrompi ed esci
		if json_body == null or typeof(json_body) != TYPE_DICTIONARY:
			SWLogger.error("SilentWolf: Risposta del server non valida (non è un JSON). Raw body: " + raw_body)
			sw_get_scores_complete.emit({"success": false, "error": "Invalid server response"})
			return
